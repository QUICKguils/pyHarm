...
class ABCSystem(abc.ABC):
    ...
    The system is responsible for assembling the Residual and Jacobian of a list of elements.

    Args:
        idata (dict): A dictionary containing the system parameters, the local coordinate systems,
          the substructures the kinematic conditions and the connectors.
    ...
    def __init__(self, idata: dict) -> None:
        ...
        self.LE_extforcing = [
            e for e in self.LE if ((not e.flag_nonlinear) and (e.flag_extforcing))
        ]
        self.LE_linear = [
            e for e in self.LE if ((not e.flag_nonlinear) and (not e.flag_extforcing))
        ]
        self.LE_nonlinear_dlft = [e for e in self.LE if ((e.flag_nonlinear) and (e.flag_DLFT))]
        self.LE_nonlinear_nodlft = [
            e for e in self.LE if ((e.flag_nonlinear) and not (e.flag_DLFT))
        ]
    ...
    def _maincarateristics(self, idata: dict) -> None:
        ...
        Attributes:
            system_options (dict): dictionary containing other options for creation of the system
              class.
            nh (int): number of harmonics.
            nti (int): number of time steps.
            adim (bool): adimension the equations using the characteristic length and angular
              frequency.
            adim_options (dict): contains the characteristic length and angular frequency.
            lc (float): characteristic length.
            wc (float): characteristic angular frequency.
        ...
        self.nh = self.system_options["nh"]
        self.nti = self.system_options["nti"]
        self.adim_options = getCustomOptionDictionary(
            self.system_options.get("adim", dict()), self.default["adim"]
        )
        self.adim = self.adim_options["status"]
        if not self.adim:
            self.adim_options["lc"] = 1.0
            self.adim_options["wc"] = 1.0
        self.lc = self.adim_options["lc"]
        self.wc = self.adim_options["wc"]

    def _complete_expl_dofs(self) -> None:
        """
        Modify the explicit dof vector according to the presence of kinematic conditions or
        non-linear connexions on certain dofs.
        ...
        Attributes:
            ...
            ndofs_solve (int): number of degree of freedom that are to be solved.
        """
        # add a columns of bool into the explicit dof vector telling
        # if a dof is coonected to a nonlinear element
        ...
        if len(self.LC) != 0:
            kc_dofs[
                np.unique(np.concatenate([(e.Pslave @ e.indices).astype(int) for e in self.LC]))
            ] = 1
    ...
    def _buildElements(self, idata: dict) -> None:
        ...
        Args:
            idata (dict): A dictionary containing the system parameters, the local coordinate
              systems, the substructures the kinematic conditions and the connectors.
        ...
        for name, data in idata["substructures"].items():
            self.LS.append(generate_substructure(self.nh, name, data))
        ...
        for sub in self.LS:
            for name, data in sub.connectors.items():
                self.LE.append(generateElement(self.nh, self.nti, name, data, self.dict_CS))
        ...
        for sub in self.LS:
            for name, data in sub.kinematics.items():
                self.LC.append(generateKinematic(self.nh, self.nti, name, data, self.dict_CS))
    ...
    def _get_expl_dofs_into_solver(self) -> pd.DataFrame:
        """
        Returns the explicit dof DataFrame that has to be solved after applying the kinematic
        condtions.

        Returns:
            pd.DataFrame: Cut explicit dof DataFrame to be solved.
        """
        return self.expl_dofs[self.expl_dofs["KC"] != 1].reset_index(drop=True)
    ...
    def _complete_x(self, list_of_kine: list[ABCKinematic], x: np.ndarray, **kwargs) -> np.ndarray:
        """
        Apply the list of kinematic conditions and returns a vector of displacement to add to the
        ...
        Returns:
            np.ndarray: vector of displacement to add in order to obtain the full size displacement
              vector with kinematic conditions.
        """
    ...
    def get_full_disp(self, q: np.ndarray) -> np.ndarray:
        ...
        Args:
            q (list[ABCKinematic]): Reduced size displacement vector without kinematic conditions
              applied.
    ...
    def _complete_R(self, list_of_kine: list[ABCKinematic], R, x, **kwargs) -> np.ndarray:
        """
        Apply the list of kinematic conditions and returns a vector of residual to add to the full
        size residual vector.
    ...
    def _complete_J(
        ...
        """
        Apply the list of kinematic conditions and returns a jacobian matrices to add to the full
        size jacobian matrices.

        Args:
            list_of_kine (list[ABCKinematic]): List of kinematic conditions to be applied.
            Jx (np.ndarray): Full size jacobian matrix with respect to displacement without
              kinematic conditions applied.
            Jom (np.ndarray): Full size jacobian matrix with respect to angular frequency without
              kinematic conditions applied.
            x (np.ndarray): Full size displacement vector without kinematic conditions applied.
    ...
    def __post_init__(self) -> None:
        """
        Method that can be complete in subclasses in order to facilitate a add during the
        ...
        for e in self.LE:
            ...
            if e.factory_keyword == "substructure":
                ...
                if e.flag_elemtype == 2:
                    djdx, _ = e.evalJacobian(
                        x[:-1], 1.0, **kwargs
                    )  # legit "magic value" to obtain nabla² @ kronM
    ...
    def Residual(self) -> np.ndarray:
        """
        Abstract method that is completed in each subclass and responsible to compute the residual
        vector of the whole system.
        """
        pass
    ...
    def Jacobian(self) -> tuple[np.ndarray]:
        """
        Abstract method that is completed in each subclass and responsible to compute the jacobian
        matrices of the whole system.
        """
        pass
