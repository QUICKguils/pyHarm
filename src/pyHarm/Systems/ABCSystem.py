...
class ABCSystem(abc.ABC):
    ...
    default = {"nh": 1, "nti": 128, "adim": {"status": False, "lc": 1.0, "wc": 1.0}}
    ...
    def __init__(self, idata: dict) -> None:
        ...
        self.LE_nonlinear_dlft = [
            e for e in self.LE if ((e.flag_nonlinear) and (e.flag_DLFT))
        ]
    ...
    def _maincarateristics(self, idata: dict) -> None:
        ...
        Attributes:
            ...
            wc (float): characteristic angular frequency.
        """
        self.system_options = getCustomOptionDictionary(
            idata.get("system", dict()), self.default
        )
    ...
    def _complete_expl_dofs(self) -> None:
        ...
        if len(self.LC) != 0:
            kc_dofs[
                np.unique(
                    np.concatenate(
                        [(e.Pslave @ e.indices).astype(int) for e in self.LC]
                    )
                )
            ] = 1
    ...
    def _buildElements(self, idata: dict) -> None:
        ...
        max_ndof = np.max(
            np.array([s["ndofs"] for s in idata["substructures"].values()])
        )
        ...
        for sub in self.LS:
            for name, data in sub.connectors.items():
                self.LE.append(
                    generateElement(self.nh, self.nti, name, data, self.dict_CS)
                )
    ...
    def _generate_system_dof_DataFrame(
        self,
    ):
    ...
    def _get_expl_dofs_into_solver(
        ...
        """
        expl_dofs_into_solver = self.expl_dofs[self.expl_dofs["KC"] != 1].reset_index(
            drop=True
        )
