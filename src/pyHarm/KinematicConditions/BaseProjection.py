...
class BaseProjection(ABCKinematic):
    ...
    def __post_init__(
        ...
        if "phi_inv" not in self.data.keys():
            ...
            self.phi_inv = np.kron(np.eye(2 * self.nh + 1), self.data["phi_inv"])
    ...
    def generateIndices(self, expl_dofs: pd.DataFrame):
        ...
        i_master = ed[(ed["sub"] == self.subs[1])].index
        i_slave = ed[(ed["sub"] == self.subs[0])].index
        self.Pmaster, self.Pslave = (
            np.eye(len(self.indices))[i_master, :],
            np.eye(len(self.indices))[i_slave, :],
        )
