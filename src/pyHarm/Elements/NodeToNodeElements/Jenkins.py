...
def JenkinsCorLoop_jax(x_k, k, DFT, limit_friction_force):
    ...
    def function_to_loop(t, state):
        ...
        z0 = jax.lax.cond(
            slip, true_fn, false_fn, x_k, z0, k, limit_friction_force, delta_x, dx_norm
        )
        z = z.at[:, t].set(z0)
        return (z0, z, fpre)
        ...
        for t in range(nti):
            ...
            if slip:
                ...
                for dir_j in range(x_k.shape[0]):
                    dvdx_j = (
                        (ddelta_xdx[:, dir_j, :]) / dx_norm
                        - (
                            delta_x.reshape(-1, 1)
                            @ (delta_x @ ddelta_xdx[:, dir_j, :]).reshape(1, -1)
                        )
                        / dx_norm**3
                    )
                    dz0[:, dir_j, :] = (
                        dx_kdx_j[:, dir_j, t, :] - limit_friction_force[t] * dvdx_j / k
                    )
