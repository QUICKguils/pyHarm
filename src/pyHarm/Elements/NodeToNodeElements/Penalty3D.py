...
def evalResidual_jax(x, om, Pdir, Pslave, Pmaster, mu, N0, g, k_n, k_t, DFT, DTF):
    # Normal contact
    R_N, f_time_n = PenalGapResidual_jax(
        x, om, Pdir[[0], :, :], Pslave, Pmaster, g, k_n, DFT, DTF, N0
    )

    # Tangential contact
    R_T = JenkinsResidual_jax(
        x, om, Pdir[[1, 2], :, :], Pslave, Pmaster, mu, N0, k_t, DFT, DTF, f_time_n
    )
