...
@jax.jit
def PenalGapResidual_jax(x, om, Pdir, Pslave, Pmaster, g, k, DFT, DTF, N0=0.0):
    R = jnp.zeros((len(x),))
    nti = jnp.shape(DFT)[1]
    ...
    for dir1 in range(jnp.shape(Pdir)[0]):
        ...
        def false_fn(x_d1, g, N0, k):
            return x_d1 <= N0 / k, 0.0

        contact, g = jax.lax.cond(N0 == 0.0, true_fn, false_fn, x_d1, g, N0, k)
...
@njit(cache=True)
def PenalGapResidual(x, om, Pdir, Pslave, Pmaster, g, k, DFT, DTF, N0=0.0):
    R = np.zeros((len(x),))
    nti = DFT.shape[1]
...
@njit(cache=True)
def PenalGapJacobian(x, om, Pdir, Pslave, Pmaster, g, k, DFT, DTF, N0=0.0):
