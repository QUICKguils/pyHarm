...
@njit(cache=True)
def DLFTUniGapJacobian(
    x, om, Rlin, dJdxlin, dJdomlin, nbSub, Pdir, Pslave, Pmaster, g, eps, DFT, DTF
):
    ...
    for dir1 in range(Pdir.shape[0]):
        ...
        dlam_opt_t = (
            (1 / nbSub * dJdxlin @ (Pslave - Pmaster).T @ Pdir[dir1, :, :].T)
            + eps * ((Pslave - Pmaster).T @ Pdir[dir1, :, :].T)
        ) @ DFT
        for o in overshoot:
            dlam_opt_t[:, o] = 0
