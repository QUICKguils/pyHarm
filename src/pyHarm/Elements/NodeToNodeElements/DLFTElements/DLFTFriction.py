...
def DLFTFrictionResidualCorLoop_jax(lambda_opt_t, lambda_pre_t, limit_friction_force):
    ...
    def function_to_loop(t, state):
        ...
        def true_fn(lambda_cor_tm1, limit_friction_force, lambda_pre_t1):
            return lambda_cor_tm1 + lambda_pre_t1 * (
                1 - limit_friction_force / jnp.sqrt(jnp.sum(lambda_pre_t1**2))
            )

        def false_fn(lambda_cor_tm1, limit_friction_force, lambda_pre_t1):
            return lambda_cor_tm1

        lambda_cor_t = lambda_cor_t.at[:, t].set(
            jax.lax.cond(
                slip,
                true_fn,
                false_fn,
                lambda_cor_t[:, t - 1],
                limit_friction_force[t],
                lambda_pre_t[:, t],
            )
        )
...
@jax.jit
def DLFTFrictionResidual_jax(
    x, om, Rlin, nbSub, Pdir, Pslave, Pmaster, mu, N0, eps, DFT, DTF, f_time_n
):
    ...
    for dir_k in range(jnp.shape(Pdir)[0]):
        lambda_opt_t = lambda_opt_t.at[dir_k, :].set(
            (
                -1 / nbSub * (Pdir[dir_k, :, :] @ (Pslave - Pmaster) @ Rlin)
                + eps * (Pdir[dir_k, :, :] @ (Pslave - Pmaster) @ x)
            )
            @ DFT
        )
    lambda_pre_t = lambda_opt_t - jnp.zeros((jnp.shape(Pdir)[0], nti))
    lambda_cor_t, lambda_pre_t, slip_all = DLFTFrictionResidualCorLoop_jax(
        lambda_opt_t, lambda_pre_t, limit_friction_force
    )
...
jaxf = jax.jacfwd(DLFTFrictionResidual_jax)
jaxf = jax.jit(jaxf)
    ...
    for _ in range(3):
        for t in range(nti):
            ...
            if STUCK:
                ...
                lambda_cor_t[:, t] = lambda_cor_t[:, t - 1] + lambda_pre_t[:, t] * (
...
@njit(cache=True)
def DLFTFrictionJacobianCorLoop(
    dlamdx_opt_t_ss, dlamdx_opt_t_sm, dlamdom_opt_t, lambda_pre_t, mu, N0
):
        ...
        for t in range(nti):
            ...
            if STUCK:
                ...
                for dir_k in range(dlamdx_opt_t_ss.shape[0]):
                    ...
                    dlamdx_cor_t_ss[dir_k, :, :, t] = (
                        ...
                        * (1 - mu * N0 / np.sqrt(np.sum(lambda_pre_t[:, t] ** 2)))
                        + mu
                        * N0
                        * lambda_pre_nicelyshaped
                        @ sum_directions_ss
                        / np.sqrt(np.sum(lambda_pre_t[:, t] ** 2)) ** 3
                    )

                    # dlamdx_cor_t_sm
                    sum_directions_ms = (
                        lambda_pre_t[:, t] @ dlamdx_pre_t_sm[dir_k, :, :, t]
                    ).reshape(1, -1)
                    dlamdx_cor_t_sm[dir_k, :, :, t] = (
                        dlamdx_cor_t_sm[dir_k, :, :, t - 1]
                        + dlamdx_pre_t_sm[dir_k, :, :, t]
                        * (1 - mu * N0 / np.sqrt(np.sum(lambda_pre_t[:, t] ** 2)))
                        + mu
                        * N0
                        * lambda_pre_nicelyshaped
                ...
                dlamdom_cor_t[:, t] = (
                    ...
                    * N0
                    * lambda_pre_t[:, t]
                    * (lambda_pre_t[:, t] @ dlamdom_pre_t[:, t])
                    / np.sqrt(np.sum(lambda_pre_t[:, t] ** 2)) ** 3
                )

            dlamdx_pre_t_ss[:, :, :, tp1] = (
                dlamdx_opt_t_ss[:, :, :, tp1] - dlamdx_cor_t_ss[:, :, :, t]
            )
            dlamdx_pre_t_sm[:, :, :, tp1] = (
                dlamdx_opt_t_sm[:, :, :, tp1] - dlamdx_cor_t_sm[:, :, :, t]
            )
...
@njit(cache=True)
def DLFTFrictionJacobian(
    x, om, Rlin, dJdxlin, dJdomlin, nbSub, Pdir, Pslave, Pmaster, mu, N0, eps, DFT, DTF
):
    ...
    _, lambda_opt, lambda_cor, lambda_pre = DLFTFrictionResidual(
        x, om, Rlin, nbSub, Pdir, Pslave, Pmaster, mu, N0, eps, DFT, DTF
    )
    for dir_k in range(Pdir.shape[0]):
        ...
        dlamdom_opt_t[dir_k, :] = (
            -1 / nbSub * Pdir[dir_k, :, :] @ (Pslave - Pmaster) @ dJdomlin
        ).reshape(
            -1,
        ) @ DFT
    dlamdx_cor_t_ss, dlamdx_cor_t_ms, dlamdom_cor_t = DLFTFrictionJacobianCorLoop(
        dlamdx_opt_t_ss, dlamdx_opt_t_ms, dlamdom_opt_t, lambda_pre, mu, N0
    )
    for dir_k in range(Pdir.shape[0]):
        for dir_j in range(Pdir.shape[0]):
            dlam_h_ss = (
        ...
        dJdom += (
            (Pslave - Pmaster).T
            @ Pdir[dir_k, :, :].T
            @ ((dlamdom_opt_t[dir_k, :] - dlamdom_cor_t[dir_k, :]) @ DTF)
        )
    return dJdx, dJdom
...
class DLFTFriction(DLFTElement):
    ...
    def _evalJaco_ana(self, xg, om, Rglin, dJgdxlin, dJgdomlin, *args):
        ...
        dJdx, dJdom = DLFTFrictionJacobian(
            x,
            om,
            Rlin,
            dJdxlin,
    ...
    def evalResidual(self, xg, om, Rglin=None):
        ...
        x = xg[self.indices]
        Rlin = Rglin[self.indices]
        self.R, _, _, _ = DLFTFrictionResidual(
            x,
            om,
            Rlin,
            float(self.nbSub),
