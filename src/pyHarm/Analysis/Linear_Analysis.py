...
class Linear_Analysis(ABCAnalysis):
    ...
    def initialize(self):
        ...
        if len(self.system.LE_nonlinear_dlft) != 0:
            ...
            K_assembled = self.system._get_assembled_stiffness_matrix(
                x0, **{"Rglin": Rlin, "dJgdxlin": None, "dJgdomlin": None}
            )
        else:
    ...
    def compute_linear_FRF(self, K, M, phi):
        ...
        # Option 2: Modal damping
            ...
            Cg = (
                2
                * damping["modal"]["xi"]
                * np.sqrt(np.diag(np.diag(Kg)))
                @ np.sqrt(np.diag(np.diag(Mg)))
            )
        ...
        dof_ex = expl_dofs[
            (expl_dofs["harm"] == 0)
            & (expl_dofs["sub"] == sub)
            & (expl_dofs["node_num"] == node_num)
            & (expl_dofs["dof_num"] == dof_num)
        ].index[0]
        F = np.zeros((len(K), 1))
