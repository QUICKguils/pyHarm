...
class AllgowerPreconditioner(ABCReductor):
    ...
    This reductor is based on the preconditioning proposal made in Allgower et
    al. book named:
        - Numerical Continuation Methods: An Introduction.
    It uses QR decomposition of the Jacobian matrix and normalises the Jacobian
    using the diagonal terms in the R matrix.
    ...
    def update_reductor(self, xpred: np.ndarray, J_f: np.ndarray, *args):
        """
        Computes a preconditioning scaling matrix based on the QR decomposition
        of the jacobian.
    ...
    def expand(self, q: np.ndarray) -> np.ndarray:
        """
        Does nothing for this class, as the preconditioner does not change size
        of the system.
    ...
    def reduce_vector(self, R: np.ndarray) -> np.ndarray:
        """
        From original residual vector, performs the transformation to get the
