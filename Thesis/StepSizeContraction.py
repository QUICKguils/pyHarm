...
class StepSizeContraction(ABCStepSizeRule):
    """Dynamic stepsizer based on the contraction rate of the Newton-Raphson corrections.
    Attributes:
        default_options (dict): contains default step size options concerning
          consecutive_accept if not provided during creation.
         (int): number of consecutive accept before
          increasing step size.
    """

    name = "accepted step size adaptation"
    factory_keyword: str = "myacceptance"
    default_options = {"kappa_max": 0.6}
