...
INPUT_WITH_ELEMENTS["connectors"]["E0"] = {
    "type": "LinearSpring",
    "connect": {"sub": [0]},
    "dirs": [0],
    "k": 1.0,
}
INPUT_WITH_ELEMENTS["connectors"]["E1"] = {
    "type": "LinearSpring",
    "connect": {"sub": [0], "INTERNAL": [1]},
    "dirs": [0],
    "k": 1.0,
}
INPUT_WITH_ELEMENTS["kinematics"]["K0"] = {
    "type": "GOdisplacement",
    "connect": {"sub": [1]},
    "dirs": [0],
    "amp": 1.0,
...
def test_ABCSystem__buildElements(mock_S_noinit: ABCSystem):
    ...
    input_with_linearspring["connectors"]["E0"] = {
        "type": "LinearSpring",
        "connect": {"sub": [0]},
        "dirs": [0],
        "k": 1.0,
    }
    input_with_linearspring["connectors"]["E1"] = {
        "type": "LinearSpring",
        "connect": {"sub": [0], "INTERNAL": [1]},
        "dirs": [0],
        "k": 1.0,
    }
    input_with_linearspring["kinematics"]["K0"] = {
        "type": "GOdisplacement",
        "connect": {"sub": [1]},
        "dirs": [0],
        "amp": 1.0,
...
def test_ABCSystem__complete_expl_dofs(mock_S_noinit: ABCSystem):
    ...
    mock_S_noinit.LE_nonlinear_nodlft = [
...
def test_ABCSystem__expand_q(mock_S: ABCSystem):
    x = mock_S._expand_q(np.array([1.0, 2.0, 3.0, 100.0]))
    assert (x[np.array([0, 2, 4])] == np.array([1.0, 2.0, 3.0])).all()
    assert (x[np.array([1, 3, 5])] == np.array([0.0, 0.0, 0.0])).all()
...
def test_ABCSystem__complete_x(mock_S: ABCSystem):
    ...
    assert (x[np.array([0, 2, 4])] == np.array([1.0, 2.0, 3.0])).all()
    assert (x[np.array([1, 3, 5])] == np.array([0.0, 1.0, 0.0])).all()


@pytest.mark.unit
@pytest.mark.all
def test_ABCSystem_get_full_disp(mock_S: ABCSystem):
    x = mock_S.get_full_disp(np.array([1.0, 2.0, 3.0, 100.0]))
    assert (x[np.array([0, 2, 4])] == np.array([1.0, 2.0, 3.0])).all()
    assert (x[np.array([1, 3, 5])] == np.array([0.0, 1.0, 0.0])).all()
...
def test_ABCSystem__get_assembled_stiffness_matrix(mock_S: ABCSystem):
    K = mock_S._get_assembled_stiffness_matrix(
        np.zeros(
            7,
        )
    )
    assert K.shape == (6, 6)
    assert (K == np.kron(np.eye(3), np.array([[2.0, -1.0], [-1.0, 1.0]]))).all()


INPUT_WITH_MASS = copy.deepcopy(INPUT_WITH_ELEMENTS)
INPUT_WITH_MASS["connectors"]["mass"] = {
    "type": "GOElement",
    "connect": {"sub": [0]},
    "dirs": [0],
    "dto": 2,
...
def test_ABCSystem__get_assembled_mass_matrix(mock_S_withmass: ABCSystem):
    M = mock_S_withmass._get_assembled_mass_matrix(
        np.zeros(
            7,
        )
    )
    assert M.shape == (6, 6)
    verif = np.kron(np.eye(3), np.array([[-1.0, 0.0], [0.0, 0.0]]))
    verif[0] = 0.0
    assert (M == verif).all()
