...
def test_ABCPredictor_getPointerToSolution(mock_abcpredictor: ABCPredictor) -> None:
    ...
    assert mock_abcpredictor.get_last_solution(sollist, k_imposed=0) == SOL_ACCEPT
    assert mock_abcpredictor.get_last_solution(sollist, k_imposed=1) == SOL_NOTACC
    assert mock_abcpredictor.get_last_solution(sollist, k_imposed=None) == SOL_ACCEPT
