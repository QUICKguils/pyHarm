...
def get_sector_rigid_mat_corr(kg, kb1, kb2):
    K = np.array([[kg + kb1, -kb1, 0], [-kb1, kb1 + kb2, -kb2], [0, -kb2, kb2]])
    return K
...
def get_families_from_diam_corr(pd_diam):
    ...
    loc_fam_cut = np.concatenate(
        [
            np.array([-1]),
            np.where(np.sign(np.diff(diam_num)) < 0)[0],
            np.array([len(diam_num)]),
        ]
    )
    list_fam = []
...
def make_system_input_corr(nh=1, nti=128):
    System = {"type": "Base", "nh": nh, "nti": nti}
    return System
...
def make_ring_sector_sub_dict_corr(Mrs):
    ...
    Sector_ring = {"matrix": matrix, "ndofs": 1}
    return Sector_ring
