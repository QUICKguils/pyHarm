...
def ElementaryMatrixResidual_corr(
    x: np.ndarray, om: float, kronAe: np.ndarray, dto: int
) -> np.ndarray:
    R = (om**dto * kronAe) @ x
    return R


def ElementaryMatrixJacobian_corr(
    x: np.ndarray, om: float, kronAe: np.ndarray, dto: int
...
# 1.1 correction functions :
def add_EM_connectors_corr(
    INP: dict, number_elements: int, Ae: np.ndarray, dto: int, prefix_name: str
) -> dict:
    """
...
def make_modal_analysis(
    ...
    phi = phi[:, ii_sorted]
    scaler = np.sqrt(
        np.diag(1 / np.diag(phi.T @ Mcond @ phi))
    )  # the vectors are scaled according to mass matrix
...
def get_phi_cb_corr(phy_dyn, phy_stat):
    phi = np.block(
        [
            [
                np.eye(phy_stat.shape[1]),
                np.zeros((phy_stat.shape[1], phy_dyn.shape[1])),
            ],
            [phy_stat, phy_dyn],
        ]
    )
...
def get_gen_matrices_corr(Mp, Cp, Kp, phi_CB):
    list_A = []
    for A in (Mp, Cp, Kp):
        list_A.append(phi_CB.T @ A @ phi_CB)
    return tuple(list_A)
